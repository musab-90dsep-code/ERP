...
import { ArrowLeft, ArrowDownLeft, ArrowUpRight, TrendingDown, Banknote, Search, Filter } from 'lucide-react';
...
function CashbookContent() {
  ...
  const currentData = activeTab === 'inflow' ? inflowLog : outflowLog;
  ...
  return (
    <div className="pb-24 w-full min-h-screen bg-[#0b0f1a] px-6 pt-6 max-w-[1400px] mx-auto" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      {/* ── DATA TABLE ── */}
      <div className="bg-[#131929] rounded-2xl border border-[rgba(255,255,255,0.04)] shadow-[0_8px_32px_rgba(0,0,0,0.4)] overflow-hidden">
        {loading ? (
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-[rgba(255,255,255,0.02)]">
                {filteredData.map((item, idx) => (
                  <tr key={idx} className="hover:bg-[#1a2235]/40 transition-colors group">
                    <td className="py-4 px-6">
                      <p className="text-sm font-medium text-[#e8eaf0]">{new Date(item.date).toLocaleDateString()}</p>
                    </td>
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${activeTab === 'inflow' ? 'bg-emerald-500/10' : 'bg-red-500/10'}`}>
