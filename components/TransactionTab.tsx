...
import { Plus, Trash2, DollarSign, Calendar, FileText, User, Receipt, Search, X, Banknote, Wallet, CreditCard, CheckCircle } from 'lucide-react';
...
export default function TransactionTab({ employees, transactions, attendance = [], fetchTransactions, handleDelete }: TransactionTabProps) {
  ...
  const [allowanceModal, setAllowanceModal] = useState<{ isOpen: boolean; type: 'daily' | 'monthly' }>({ isOpen: false, type: 'daily' });
  ...
  // ── CORE LOGIC: Exact Date and Month Tracking ──
  const generatePreview = (type: 'daily' | 'monthly', monthStr: string) => {
    const list: PreviewRow[] = [];
    const formattedMonthStr = new Date(monthStr + '-01').toLocaleString('default', { month: 'short', year: 'numeric' });

    employees.forEach(emp => {
      if (type === 'daily') {
        if (!emp.daily_allowance || emp.daily_allowance <= 0) return;

        // 1. Get all exact dates already paid to this employee
        const paidDates = new Set<string>();
        transactions.forEach(t => {
          if (t.employee === emp.id && t.type === 'allowance' && t.note) {
            const match = t.note.match(/\[Paid Dates: (.*?)\]/);
            if (match && match[1]) {
              match[1].split(',').forEach((d: string) => paidDates.add(d.trim()));
            }
          }
        });

        // 2. Get all dates they were present this month
        ...
        const presentDates = monthlyAttendance.map(a => a.date).sort();

        // 3. Subtract paid dates from present dates
        const unpaidDates = presentDates.filter(d => !paidDates.has(d));

        // 4. Create preview row if they have unpaid days left
        if (unpaidDates.length > 0) {
          const pendingDays = unpaidDates.length;
          list.push({
            employee: emp.id,
            name: emp.name,
            role: emp.role,
            photo: emp.profile_image_url,
            eligibleDays: pendingDays,
            unpaidDates: unpaidDates, // Save the exact array of dates
            baseRate: emp.daily_allowance,
            finalAmount: pendingDays * emp.daily_allowance,
            note: `Daily Allowance - ${formattedMonthStr}`,
            systemTag: `[Paid Dates: ${unpaidDates.join(',')}]` 
          });
        }
      } else {
        // --- MONTHLY ALLOWANCE LOGIC ---
        ...
        if (!isAlreadyPaid) {
          list.push({
            ...
            note: `Monthly Fixed Allowance - ${formattedMonthStr}`,
  ...
  useEffect(() => {
    if (allowanceModal.isOpen) {
      generatePreview(allowanceModal.type, selectedMonth);
    }
  }, [allowanceModal.isOpen, allowanceModal.type, selectedMonth, employees, attendance, transactions]);
    ...
    <div className="space-y-6 sm:space-y-8 max-w-[1400px] mx-auto pb-10 px-2 sm:px-0">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 bg-[#131929] p-6 rounded-2xl border border-[rgba(255,255,255,0.04)] shadow-[0_4px_24px_rgba(0,0,0,0.5)]">
        ...
        <div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3 w-full xl:w-auto">
          <button 
            onClick={() => setAllowanceModal({ isOpen: true, type: 'daily' })}
            className="flex-1 sm:flex-none justify-center bg-[#1a2235] text-blue-400 px-5 py-3 rounded-lg text-[11px] uppercase tracking-widest font-black border border-[rgba(96,165,250,0.2)] shadow-sm hover:bg-[rgba(96,165,250,0.1)] transition-colors"
          >
            Daily Allowances (Pending)
          </button>
          <button 
            onClick={() => setAllowanceModal({ isOpen: true, type: 'monthly' })} 
            className="flex-1 sm:flex-none justify-center bg-[#1a2235] text-purple-400 px-5 py-3 rounded-lg text-[11px] uppercase tracking-widest font-black border border-[rgba(168,85,247,0.2)] shadow-sm hover:bg-[rgba(168,85,247,0.1)] transition-colors"
          >
            Monthly Allowances
          </button>
          <button 
            onClick={() => setShowForm(true)} 
            className="flex-1 sm:flex-none justify-center bg-gradient-to-br from-[#c9a84c] to-[#f0c040] text-[#0a0900] px-6 py-3 rounded-lg flex items-center gap-2 hover:opacity-90 font-extrabold shadow-[0_4px_16px_rgba(201,168,76,0.3)] transition-all"
          >
          ...
          <div className="bg-[#131929] rounded-2xl shadow-[0_16px_64px_rgba(0,0,0,0.9)] border border-[rgba(201,168,76,0.3)] w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <div className="flex justify-between items-center p-6 border-b border-[rgba(255,255,255,0.06)] bg-[#1a2235]">
              <h2 className="text-base font-black text-white uppercase tracking-widest flex items-center gap-3">
                ...
                Review Unpaid {allowanceModal.type === 'daily' ? 'Daily' : 'Monthly'} Allowances
              </h2>
            ...
            <div className="p-6 bg-[#0b0f1a] border-b border-[rgba(255,255,255,0.04)] flex flex-wrap gap-4 items-center justify-between">
              <div>
                <label className={labelClass}>Select Target Month</label>
            ...
            <div className="flex-1 overflow-auto custom-scrollbar p-6">
              {previewList.length === 0 ? (
                <div className="text-center py-16">
                  ...
                  <p className="font-bold text-[#8a95a8]">No unpaid allowances found for this month.</p>
                  <p className="text-xs text-[#4a5568] mt-2">All eligible allowances for {new Date(selectedMonth + '-01').toLocaleString('default', { month: 'short', year: 'numeric' })} have already been paid.</p>
                </div>
              ) : (
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead className="bg-[#1a2235] border-b border-[rgba(255,255,255,0.06)] sticky top-0 z-10">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-[10px] font-extrabold text-[#8a95a8] uppercase tracking-widest text-right">Base Rate</th>
                        <th className="px-4 py-3 text-[10px] font-extrabold text-[#c9a84c] uppercase tracking-widest text-right w-32">Final Amount (৳)</th>
                    ...
                    <tbody className="divide-y divide-[rgba(255,255,255,0.02)]">
                      {previewList.map(row => (
                        <tr key={row.employee} className="hover:bg-[rgba(255,255,255,0.02)] transition-colors">
                          ...
                          <td className="px-4 py-3 text-right font-medium text-[#8a95a8]">৳{row.baseRate}</td>
                          <td className="px-4 py-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="bg-[#131929] rounded-2xl p-6 border border-[rgba(255,255,255,0.04)] shadow-[0_4px_24px_rgba(0,0,0,0.5)] flex items-center gap-5 relative overflow-hidden group">
          <div className="absolute -right-4 -bottom-4 opacity-5 group-hover:scale-110 transition-transform duration-500">
            <Wallet className="w-32 h-32 text-blue-500" />
          </div>
          <div className="bg-[rgba(96,165,250,0.1)] p-4 rounded-xl border border-[rgba(96,165,250,0.2)] text-blue-400 relative z-10"><DollarSign className="w-6 h-6"/></div>
          <div className="relative z-10">
            <p className="text-[10px] font-bold text-[#8a95a8] uppercase tracking-widest mb-1">Total Disbursed</p>
            <h3 className="text-2xl font-black text-white"><span className="text-[#8a95a8] font-bold mr-1">৳</span>{totalPaid.toLocaleString()}</h3>
          </div>
        </div>
        ...
        <div className="bg-[#131929] rounded-2xl p-6 border border-[rgba(255,255,255,0.04)] shadow-[0_4px_24px_rgba(0,0,0,0.5)] flex items-center gap-5 relative overflow-hidden group">
          ...
          <div className="bg-[rgba(52,211,153,0.1)] p-4 rounded-xl border border-[rgba(52,211,153,0.2)] text-emerald-400 relative z-10"><Receipt className="w-6 h-6"/></div>
          <div className="relative z-10">
            <p className="text-[10px] font-bold text-[#8a95a8] uppercase tracking-widest mb-1">Salary Paid</p>
            <h3 className="text-2xl font-black text-white"><span className="text-[#8a95a8] font-bold mr-1">৳</span>{totalSalary.toLocaleString()}</h3>
          </div>
        </div>
        ...
        <div className="bg-[#131929] rounded-2xl p-6 border border-[rgba(255,255,255,0.04)] shadow-[0_4px_24px_rgba(0,0,0,0.5)] flex items-center gap-5 relative overflow-hidden group">
          ...
          <div className="bg-[rgba(251,191,36,0.1)] p-4 rounded-xl border border-[rgba(251,191,36,0.2)] text-yellow-400 relative z-10"><FileText className="w-6 h-6"/></div>
          <div className="relative z-10">
            <p className="text-[10px] font-bold text-[#8a95a8] uppercase tracking-widest mb-1">Advances Issued</p>
            <h3 className="text-2xl font-black text-white"><span className="text-[#8a95a8] font-bold mr-1">৳</span>{totalAdvance.toLocaleString()}</h3>
          </div>
        </div>
      </div>
