...
import { Trash2, Users, UserCheck, UserX, Clock, History, CalendarPlus, DownloadCloud } from 'lucide-react';
...
export default function AttendanceTab({ employees, attendance, handleDelete }: AttendanceTabProps) {
  ...
  const handleDownloadHistory = () => {
    ...
    const tableRows = filteredAttendance.map((a, i) => {
      const empInfo = getEmployee(a.employee_id);
      const empName = empInfo?.name || 'Unknown';
      const role = empInfo?.role || 'Staff';
      const statusStyle = a.status === 'present' ? 'color: green;' : a.status === 'absent' ? 'color: red;' : 'color: orange;';

      return `
        <tr style="border-bottom: 1px solid #eee;">
          <td style="padding: 10px; font-size: 12px;">${i + 1}</td>
          <td style="padding: 10px; font-size: 12px; font-weight: bold;">${a.date}</td>
          <td style="padding: 10px; font-size: 12px; font-weight: bold;">${empName}</td>
          <td style="padding: 10px; font-size: 12px;">${role}</td>
          <td style="padding: 10px; font-size: 12px; text-transform: uppercase; font-weight: bold; ${statusStyle}">${a.status}</td>
        </tr>
      `;
    }).join('');

    let filterText = "All History";
    if (filterDate || filterEmployee) {
       filterText = "Filtered by: " + (filterDate ? "Date: " + filterDate + " " : "") + (filterEmployee ? "Employee: " + getEmployeeName(filterEmployee) : "");
    }

    const html = `
      <html>
        <head>
          <title>Attendance History - LedgerGhor</title>
          <style>
        ...
        <body>
          <div class="header">
            ...
            <h2 style="margin:5px 0; color: #555;">Attendance History</h2>
            <p style="font-size: 12px; font-weight: bold; color: #333;">${filterText}</p>
      ...
      <div className="bg-[#131929] rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.5)] border border-[rgba(255,255,255,0.04)] overflow-hidden">
        <div className="p-6 border-b border-[rgba(255,255,255,0.04)] bg-[#1a2235] flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            ...
            <button onClick={handleDownloadHistory} className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-[rgba(255,255,255,0.05)] border border-[rgba(255,255,255,0.1)] text-[#e8eaf0] text-sm font-bold shadow-sm transition hover:bg-[rgba(255,255,255,0.1)]">
